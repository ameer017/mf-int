...
function App() {
  ...
  const {
    ...
    isFetching,
  } = useGetTokensAndBalances(userAddress);
    ...
    <div className="w-full min-h-screen bg-white flex flex-col p-4">
      <div className="p-6 sticky top-0 z-10 flex flex-col justify-center items-center bg-white w-full">
        ...
        <p className="text-gray-800">
          Enter your wallet address to get your token balances on Ethereum
          Mainnet
        </p>
      </div>
      ...
      {/* cards of balances */}
      <h1 className="text-indigo-600 mt-14 text-xl font-medium text-center">
        List of Tokens
      </h1>
      <h3 className="text-gray-800 mb-3 text-sm text-center">
        Current User Account: {currentAddress}
      </h3>
      <main className=" sm:mx-auto sm:w-full sm:max-w-xl space-y-6">
        {isFetching ? (
          <h2 className="text-indigo-600 mt-10 text-base font-medium text-center">
            Fetching Tokens...
          </h2>
        ) : (
          tokens.map((token) => (
            <div
              key={token.address}
              className="rounded-md shadow-md border bg-white p-6 flex flex-col"
            >
